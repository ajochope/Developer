...
#import "Chat.h"
#import "WhasaDAO.h"
#import "WhasaAppDelegate.h"
#import "Conversacion.h"
#import "Amigo.h"

@interface WhasaChatViewController ()
@property (strong, nonatomic)  WhasaDAO *dao;
@property (strong, nonatomic) Conversacion *conversacionVO;
// VO value Object
...
- (IBAction)enviarMensaje:(id)sender {
    ...
    [_dao actualizarConversacion:_conversacion conMensaje:_mensaje.text];
...
- (void)viewDidLoad
{
   [super viewDidLoad];
    ...
    // creamos la instacion del delagate
    id appDelegate = [[UIApplication sharedApplication] delegate];
    if(! _dao){
        _dao = [[WhasaDAO alloc] initConManagedObjectContext:[appDelegate managedObjectContext]];
    }
    Amigo *amigo = [_dao crearAmigoConNombre:_nombreDelContacto yFechaNacimiento:nil];
    _conversacionVO = [_dao crearConversacionConAmigo:amigo yTitulo:_navItem.prompt];
    
    if([_conversacionVO.mensaje count] > 0){
        _conversacionAux addObjectsFromArray:[_conversacionVO mensajes]:allObjects;
    }
